...
/*********************************************************
CONVERT LOG FILES TO OBJECT CONTAINERS
*********************************************************/
void get_Logs()
{
	std::string line;
...
/*********************************************************
OUTPUT TO LOG FILES AND COMMAND FILES
*********************************************************/
void to_file()
{
	...
	//Generate commands to diff 2 commits
	file.open("result/commit.cmd");
	file1.open("result/List.txt");
	if(file.is_open() && file1.is_open())
	{
		for(int i = log_list.size()-1;i>0;i--)
		{
			year = stoi(log_list[i].date.substr(19, 5));
			if(inbetween(stoi(timeline.start_year),stoi(timeline.end_year),year))
			{
				Exact_month = log_list[i].date.substr(4, 3);
				if(inbetween(Month_Day[timeline.start_month], Month_Day[timeline.end_month], Month_Day[Exact_month]))
				{
		...
		std::cout << "failed to generate commit.cmd & List.txt"<< std::endl;
	
	//Generate Report of Each user
	file.open("result/report.txt");
	if(file.is_open())
	{
		for(auto i : users)
		{
			for(int j = 0; j<i.commits.size();j++)
			{
				year = stoi(i.commits[j].date.substr(19, 5));
				if(inbetween(stoi(timeline.start_year),stoi(timeline.end_year),year))
				{
					Exact_month = i.commits[j].date.substr(4, 3);
					if(inbetween(Month_Day[timeline.start_month], Month_Day[timeline.end_month], Month_Day[Exact_month]))
					{
						day = stoi(i.commits[j].date.substr(8, 2));
						if(inbetween(stoi(timeline.start_day),stoi(timeline.end_day),day))
						{
							cnt++;
						}
			...
			file << i.name.substr(1,i.name.size()-2) <<"\n"<< cnt << "\n";
			cnt = 0;
		}
		file.close();
	}
	else
		std::cout << "failed to generate report.txt"<< std::endl;
}
